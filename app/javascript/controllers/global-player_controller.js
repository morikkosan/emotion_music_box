...
export default class extends Controller {
  ...
  _isLoggedIn() {
    try {
      ...
      if (typeof window.CURRENT_USER_ID === "number" ||
        (typeof window.CURRENT_USER_ID === "string" && window.CURRENT_USER_ID)) return true;
    } catch (_) {}
    return false;
  }
  ...
  _showHandshakeHint() {
    ...
    el.className = "sc-handshake-hint";
  ...
  _reallyUnmute(el) {
    try {
      ...
      el.muted = false;
      if (!this._isIOS()) {
        const v = this.volumeBar
          ? Math.max(0, Math.min(1, Number(this.volumeBar.value) / 100 || 1))
          : 1;
        el.volume = v;
      }
    } catch (_) {}
  ...
  async _playViaMedia({ streamUrl, useVideo = false, resumeMs = 0 }) {
    ...
    if (isHls && !canNativeHls && window.Hls && window.Hls.isSupported()) {
      if (this.__hls) {
        try { this.__hls.destroy(); } catch (_) {}
        this.__hls = null;
      }
    ...
    if (resumeMs > 0) {
      try { el.currentTime = resumeMs / 1000; } catch (_) {}
    }
  ...
  _looksDeleted({ status, bodyText, trackJson }) {
    try {
      ...
      if (policy.includes("blocked") || policy.includes("block")) return false;
      if (state.includes("removed") || state.includes("deleted")) return true;
      if (
        trackJson &&
        Array.isArray(trackJson?.media?.transcodings) &&
        trackJson.media.transcodings.length === 0
      ) {
        return false;
      }
    } catch (_) {}
  ...
  async _resolveStreamUrl(trackUrl) {
    ...
    try {
      ...
      const trans = Array.isArray(track?.media?.transcodings)
        ? track.media.transcodings
        : [];
      if (!trans.length) {
        this._debug("no transcodings");
        throw new Error("No transcodings available");
      }

      const byProto = (p) =>
        trans.find(t => new RegExp(p, "i").test(t?.format?.protocol || ""));
      const chosen = byProto("hls") || byProto("progressive");
      if (!chosen?.url) {
        this._debug("no suitable transcoding");
        throw new Error("No suitable transcoding");
      }
      ...
      this._debug("stream locator start", {
        via: "proxy",
        proto: chosen?.format?.protocol
      });
      ...
      if (!j2?.url) {
        this._debug("no final stream url");
  ...
  connect() {
    ...
    try {
      if (!sessionStorage.getItem("__gp_nav_hooks_set")) {
        ...
        window.addEventListener("turbo:before-visit", () => {
          try {
            sessionStorage.setItem("__gp_last_nav_kind", "turbo");
            // ★変更ポイント: ナビゲーション開始時に必ずプレーヤーを止める
            if (window.__gpStopAll) window.__gpStopAll();
          } catch (_) {}
        });
        ...
        window.addEventListener("beforeunload", () => {
          try {
            sessionStorage.setItem("__gp_last_nav_kind", "hard");
            // ★変更ポイント: ハード遷移前も止めておく
            if (window.__gpStopAll) window.__gpStopAll();
          } catch (_) {}
        });

        // data-turbo="false" / target / 別オリジン / download
    ...
    try {
      ...
      if (k === "turbo") {
        ...
        // referrer が同一オリジンなら Turbo 経由とみなす
      ...
      sessionStorage.removeItem("__gp_last_nav_kind");
    } catch (_) {}
    this.__arrivedByTurbo = _arrivedByTurbo;

    // ★変更ポイント: グローバルな「全部止める」ハンドラをこのインスタンスで上書き
    try {
      window.__gpStopAll = () => {
        try { this.stopOnlyPlayer?.(); } catch (_) {}
        try { this._disposeAudio?.(); } catch (_) {}
        try {
    ...
    // デバッグ初期化
    this._debugInit();
    if (location.hash.includes("gpclear")) {
      localStorage.removeItem("playerState");
      this._debug("playerState cleared");
    }
    if (location.hash.includes("gpwidget")) {
      window.__forceWidgetOnly = true;
      this._debug("force widget only = true");
    }
    if (location.hash.includes("gpapi")) {
      window.__forceWidgetOnly = false;
      this._debug("force widget only = false");
    }
    this._debug("connect");
    ...
    this._footerGuardMO.observe(this._container() || document, {
      childList: true,
      subtree: true
    });
    this._updatePlayButton();
    ...
    // ========== ③ Turbo遷移で来たときだけ状態復元 ==========
    // 「最初のページ（外部から入り直し）」では __arrivedByTurbo が false なので復元しない
    if (this.__arrivedByTurbo) {
      if (window.SC?.Widget) {
    ...
    this.iframeElement.onload = () => {
      setTimeout(() => {
        ...
        this.widget.bind(SC.Widget.Events.READY, () => {
          ...
          this.widget.seekTo(state.position || 0);
          if (state.isPlaying && !this._needsHandshake()) this._forcePlay();
  ...
  async playFromExternal(playUrl) {
      ...
      await this._playViaApi(playUrl).catch((err) => {
        if (err?.__deleted) {
          ...
          this._notify({
            icon: "warning",
            title: "再生できません",
            text: "この音楽はSoundCloud上で削除されています。"
          });
    ...
    this.iframeElement.src =
      `https://w.soundcloud.com/player/?url=${encodeURIComponent(playUrl)}&auto_play=${show ? "false" : "true"}`;
  ...
  async loadAndPlay(event) {
    ...
    if (!this.iframeElement) return;
    this.iframeElement.src =
      `https://w.soundcloud.com/player/?url=${encodeURIComponent(trackUrl)}&auto_play=${show ? "false" : "true"}`;
    if (!show) this._setIframeVisibility(false);
    ...
    this.iframeElement.onload = () => {
      setTimeout(() => {
        try { this.widget = SC.Widget(this.iframeElement); }
          ...
          return setTimeout(
            () => this.loadAndPlay({ currentTarget: el, stopPropagation() {} }),
            120
          );
        }
        this.widget.bind(SC.Widget.Events.READY, () => {
  ...
  _fallbackToWidgetFromAudio(trackUrl, el = null) {
    this._disposeAudio();
    const url = trackUrl || this._lastResolvedTrackUrl;
    if (!url) return;
    ...
    if (!this.iframeElement) return;
    this.iframeElement.src =
      `https://w.soundcloud.com/player/?url=${encodeURIComponent(this._normalizeTrackUrl(url))}&auto_play=${show ? "false" : "true"}`;
    if (!show) this._setIframeVisibility(false);
  ...
  togglePlayPause(event) {
    ...
    if (this._shouldUseApi()) {
      const a = this._ensureMedia({
        useVideo: (this.media?.tagName?.toLowerCase() === "video")
      });
      if (!a.src && !this.__hls) {
  ...
  onFinish = () => {
    ...
    if (this.isRepeat) {
      const icon =
        this.playIconTargets.find((icn) => icn.dataset.trackId == this.currentTrackId) ||
        this._q(
          `[data-track-id="${CSS.escape(String(this.currentTrackId))}"]`,
          this._container()
        );
      icon && setTimeout(
    ...
    if (nextId) {
      const icon =
        this.playIconTargets.find((icn) => icn.dataset.trackId == nextId) ||
        this._q(
          `[data-track-id="${CSS.escape(String(nextId))}"]`,
          this._container()
        );
      icon && setTimeout(
        () => this.loadAndPlay({ currentTarget: icon, stopPropagation() {} }),
  ...
  seek(e) {
    ...
    if (this._shouldUseApi() && this.audio) {
      const dur = Math.max(0, (this.audio.duration || 0) * 1000);
      if (!dur) {
        this.isSeeking = false;
        return;
      }
  ...
  changeVolume(e) {
    ...
    if (this._shouldUseApi() && this.audio) {
      try { this.audio.volume = clamped / 100; } catch (_) {}
      this.updateVolumeAria(String(clamped));
      return;
    }
  ...
  prevTrack(event) {
    ...
    if (!this.currentTrackId) {
      ...
      const icon =
        this.playIconTargets.find((icn) => icn.dataset.trackId == lastId) ||
        this._q(
          `[data-track-id="${CSS.escape(String(lastId))}"]`,
          this._container()
        );
    ...
    if (idx > 0) {
      const icon =
        this.playIconTargets.find(
          (icn) => icn.dataset.trackId == this.playlistOrder[idx - 1]
        ) ||
        this._q(
          `[data-track-id="${CSS.escape(String(this.playlistOrder[idx - 1]))}"]`,
          this._container()
        );
      icon && this.loadAndPlay({ currentTarget: icon, stopPropagation() {} });
      return;
    ...
    const icon2 =
      this.playIconTargets.find((icn) => icn.dataset.trackId == lastId2) ||
      this._q(
        `[data-track-id="${CSS.escape(String(lastId2))}"]`,
        this._container()
      );
  ...
  nextTrack(event) {
    ...
    if (!this.currentTrackId) {
      ...
      const icon =
        ...
        this._q(
          `[data-track-id="${CSS.escape(String(firstId))}"]`,
          this._container()
        );
      icon && this.loadAndPlay({ currentTarget: icon, stopPropagation() {} });
      return;
    ...
    const icon2 =
      this.playIconTargets.find((icn) => icn.dataset.trackId == firstId2) ||
      this._q(
        `[data-track-id="${CSS.escape(String(firstId2))}"]`,
        this._container()
      );
    icon2 && this.loadAndPlay({ currentTarget: icon2, stopPropagation() {} });
  }
  ...
  updatePlaylistOrder() {
    ...
    if (!this.playlistOrder.length) {
      const nodes = this._qa(
        ".playlist-container [data-track-id][data-play-url]",
        this._container()
      );
      this.playlistOrder = nodes
        .map((n) => String(n.dataset.trackId))
        .filter(Boolean);
    }
    if (this.isShuffle) {
  ...
  startWaveformAnime() {
    ...
    const animate = () => {
      ...
      for (let x = 0; x < W; x += 4) {
        const y =
          H / 2 +
          Math.sin((x + t) / 7) * (H / 2.5) * (0.7 + 0.3 * Math.sin(x / 17 + t / 13));
        ctx.lineTo(x, y);
      }
      ctx.stroke();
      ctx.restore();
  ...
  _setupIOSVolumeUI() {
    ...
    if (!document.getElementById("ios-volume-hint")) {
      ...
      const container =
        this.bottomPlayer?.querySelector(".bottom-player-extra-controls") ||
        this.bottomPlayer ||
        document.body;
  ...
  _chromeOffByBodyClass() {
    try { return document.body?.classList?.contains("page--legal-chrome-off"); }
    catch (_) { return false; }
  }

  _chromeOffByDataAttr() {
    try {
      return String(document.body?.dataset?.chromeOff || "")
        .toLowerCase() === "true";
    } catch (_) { return false; }
  }
